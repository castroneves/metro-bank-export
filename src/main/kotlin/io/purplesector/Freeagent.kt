...
class FreeAgentClient {
    fun transformAndSendStatements(statements: List<Statement>) {
...
//        val transactions = statements.flatMap { it.entries }.toSet().plus(StatementEntry(LocalDate.now(), "Test entry", "", "", "123", "12345"))
        ...
        submitStatement(oauthService, accessToken, transactions)
    }
    ...
    private fun submitStatement(
        ...
        val freeAgentEntries = transactions.map { FreeAgentStatementEntry(it.date.toString(), determineAmount(it), it.details) }

        val payload = jacksonObjectMapper().writeValueAsString(FreeAgentStatement(freeAgentEntries))

        val request = OAuthRequest(Verb.POST, "https://api.freeagent.com/v2/bank_transactions/statement?bank_account=$FREEAGENT_BANK_ACCOUNT_ID")
